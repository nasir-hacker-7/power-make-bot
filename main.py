...
def format_telegram_message(data):
    ...
    if isinstance(data, list):
        # New API format: [dt, num, cli, message]
        dt = data[0] if len(data) > 0 else ''
        num = data[1] if len(data) > 1 else ''
        cli = data[2] if len(data) > 2 else 'Unknown'
        message = data[3].strip() if len(data) > 3 else ''
    else:
...
def fetch_api_data():
    """Fetch data from API"""
    params = {"token": API_TOKEN, "records": 1000000000}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Handle direct list response (new API)
            if isinstance(data, list):
            ...
            # Handle dictionary response with status (old API format)
...
def create_message_id(data):
    ...
    if isinstance(data, list):
        ...
        dt = data[0] if len(data) > 0 else ''
        num = data[1] if len(data) > 1 else ''
        cli = data[2] if len(data) > 2 else ''
        return f"{dt}_{num}_{cli}"
    else:
        # Old API format: dictionary
